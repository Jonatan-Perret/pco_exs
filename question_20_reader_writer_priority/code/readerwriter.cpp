    ...
    void testReaderWriterPrioWriter(int nbThreadsA, int nbThreadsB, int nbIterations)
    {
        ASSERT_DURATION_LE(15, {
                               ...
                               for(int t=0; t<nbThreadsB; t++){
                                   logger() << "In main: creating thread B " << t << '\n';
                                   threadsB[t] = std::make_unique<std::thread>(taskReader,
                                                                               t,
                                                                               nbIterations,
                                                                               protocol.get(),
                                                                               checker.get());
                               ...
                               for(int t=0; t<nbThreadsA; t++){
                                   logger() << "In main: creating thread A " << t << '\n';
                                   threadsA[t] = std::make_unique<std::thread>(taskWriter,
                                                                               t,
                                                                               nbIterations,
                                                                               protocol.get(),
                                                                               checker.get());
...
TYPED_TEST_SUITE(ReaderWriterPrioWriterTest, MyTypes);
