...
TEST(Sequentialite, Standard)
{
    ...
    // Wait for T3, T4, T5 to complete before starting T6
    threads[3]->join();
    threads[4]->join();
    threads[5]->join();
    ...
    // Wait for T2 and T6 to complete before starting T7
    threads[2]->join();
    threads[6]->join();
